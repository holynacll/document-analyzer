...
async def main():
    ...
    target_empresa = Empresa(nome="Point Veículos LTDA", cnpj="04.168.812/0001-00", resumo="Empresa de veículos")
    
    # Criar o analisador
    analyzer = DocumentAnalyzer(pdf_path=pdf_path, model_name=model_name, logfire_api_key=logfire_api_key, empresa_alvo=target_empresa)
    ...
    if resultado.possui_doc_constituicao and resultado.analise:
        socios_gestores = [s.nome for s in resultado.analise.socios_com_poder_assinatura]
        print(f"\nResumo final: A empresa {resultado.analise.empresa.nome} é representado pelo(s) sócio(s) com poder(es) representativo(s): {', '.join(socios_gestores)}")
    else:
